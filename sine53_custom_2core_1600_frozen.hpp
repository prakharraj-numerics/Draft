...
/* SINE53 FROZEN permanent 2-core scheduler.
   ...
   Production threshold frozen from exact Intel Xeon 6973P-C evidence:
     ...
     - broad three-way run 33567193984, exact Xeon shards 37 and 25

   Validated behavior relevant to the frozen production rule:
     - custom2 output was bit-identical to current SINE53 on the tested grids
     - at n=1500, current SINE53 remained faster on the six-case average
     - from n=1600 through n=1900, custom2 beat current SINE53 in all six
       requested sign/range cells at every tested size on exact Xeon shard 61
     - from n=2000 through n=4500, custom2 beat current SINE53 in all six
       requested sign/range cells at every tested size on exact Xeon shard 50
     - from n=5000 through n=4000000, custom2 beat current SINE53 in all six
       requested sign/range cells at every tested size on exact Xeon shards 37
       and 25
...
class Sine53CustomPermanent2Core1600Frozen {
public:
    using fn_t = void (*)(double *, const double *, size_t);

    explicit Sine53CustomPermanent2Core1600Frozen(fn_t fn)
    ...
    ~Sine53CustomPermanent2Core1600Frozen() {
        stop_.store(true, std::memory_order_relaxed);
        generation_.fetch_add(1, std::memory_order_release);
        if (helper_.joinable()) helper_.join();
    }

    Sine53CustomPermanent2Core1600Frozen(const Sine53CustomPermanent2Core1600Frozen&) = delete;
    Sine53CustomPermanent2Core1600Frozen& operator=(const Sine53CustomPermanent2Core1600Frozen&) = delete;
