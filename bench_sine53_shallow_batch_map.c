...
{
    double t0 = shallow_now_ns();
    for (size_t r = 0; r < reps; ++r) vmdSin((MKL_INT)n, x, y, VML_HA);
    double t1 = shallow_now_ns();
    *sink += y[(n * 5u / 13u) % n];
    return (t1 - t0) / ((double)reps * (double)n);
}
...
int main(void)
{
    ...
    printf("SINE53_SHALLOW engine=%s cpu_pin=%d sizes=12 trials=%d random_batches_per_size=%d intel=oneMKL_vmdSin_VML_HA\n",
    ...
    printf("SINE53_SHALLOW_DONE engine=%s sink=%.17g\n",
