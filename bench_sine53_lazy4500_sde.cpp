...
static bool alloc_buffers(Buffers&b,size_t n){b.n=n;b.c0=SINE53_ENGINE_WIDE?2:0;b.c1=SINE53_ENGINE_WIDE?6:2;int nc=b.c1-b.c0;b.x.resize(nc);b.y.resize(nc);for(int j=0;j<nc;++j){b.x[j]=(double*)al64(n*sizeof(double));b.y[j]=(double*)al64(n*sizeof(double));if(!b.x[j]||!b.y[j])return false;fill(b.x[j],n,b.c0+j);}return true;}
static int run_sde(const std::string&which,size_t n){Buffers b;if(!alloc_buffers(b,n))return 10;Sine53BatchProductionFrozen*prod=nullptr;Sine53BatchLazy4500Candidate*temp=nullptr;if(which=="prod")prod=new Sine53BatchProductionFrozen(sine53_engine_eval);else if(which=="temp")temp=new Sine53BatchLazy4500Candidate(sine53_engine_eval);auto once=[&](){for(size_t j=0;j<b.x.size();++j){if(which=="prod")prod->run(b.y[j],b.x[j],n);else if(which=="temp")temp->run(b.y[j],b.x[j],n);else if(which=="intel")vmdSin((MKL_INT)n,b.x[j],b.y[j],VML_HA);else asm volatile(""::"r"(b.x[j]),"r"(b.y[j]),"r"(n):"memory");}};if(which!="noop")once();sine53_profile_start();once();sine53_profile_stop();if(which!="noop")g_sink+=b.y[0][(n*5u/13u)%n];int helper=temp?(temp->helper_started()?1:0):(prod?1:0);printf("S4500_SDE engine=%s stack=%s n=%zu cases=%zu helper_started=%d sink=%.17g\n",SINE53_ENGINE_WIDE?"wide":"unit",which.c_str(),n,b.x.size(),helper,(double)g_sink);delete temp;delete prod;return 0;}
int main(int argc,char**argv){if(argc!=3)return 2;pin_current(0);mkl_set_num_threads_local(1);if(!sine53_engine_init())return 3;std::string which=argv[1];size_t n=(size_t)strtoull(argv[2],nullptr,10);int rc=2;if(which=="prod"||which=="temp"||which=="intel"||which=="noop")rc=run_sde(which,n);sine53_engine_cleanup();return rc;}
