...
#include "sine53_batch_lazy_4500_candidate.hpp"
...
static int run_native(const std::string&which){
    Sine53BatchProductionFrozen*prod=nullptr;Sine53BatchLazy4500Candidate*temp=nullptr;
    if(which=="prod")prod=new Sine53BatchProductionFrozen(sine53_engine_eval);else if(which=="temp")temp=new Sine53BatchLazy4500Candidate(sine53_engine_eval);
    for(size_t n:kSizes){
        Buffers b;if(!alloc_buffers(b,n)){delete temp;delete prod;return 10;}size_t cases=b.x.size(),reps=reps_for(n,cases);
        auto once=[&](){for(size_t j=0;j<cases;++j){if(which=="prod")prod->run(b.y[j],b.x[j],n);else if(which=="temp")temp->run(b.y[j],b.x[j],n);else vmdSin((MKL_INT)n,b.x[j],b.y[j],VML_HA);}};
        uint64_t maxulp=0;if(which!="intel"){once();for(size_t j=0;j<cases;++j)vmdSin((MKL_INT)n,b.x[j],b.ref[j],VML_HA);for(size_t j=0;j<cases;++j)for(size_t i=0;i<n;++i)maxulp=std::max(maxulp,ulp_distance(b.y[j][i],b.ref[j][i]));}
        ...
        rusage ru{};getrusage(RUSAGE_SELF,&ru);double mw=median5(wall),mc=median5(cpu);int helper=temp?(temp->helper_started()?1:0):(prod?1:0);
        printf("S4500_NATIVE engine=%s stack=%s n=%zu cases=%zu reps=%zu wall_ns_el=%.9f cpu_ns_el=%.9f effective_cores=%.6f maxrss_kib=%ld helper_started=%d maxulp_vs_intel=%llu\n",SINE53_ENGINE_WIDE?"wide":"unit",which.c_str(),n,cases,reps,mw,mc,mw?mc/mw:0.0,ru.ru_maxrss,helper,(unsigned long long)maxulp);
    }
    delete temp;delete prod;printf("S4500_NATIVE_DONE engine=%s stack=%s sink=%.17g\n",SINE53_ENGINE_WIDE?"wide":"unit",which.c_str(),(double)g_sink);return 0;
}

int main(int argc,char**argv){if(argc!=2)return 2;pin_current(0);mkl_set_num_threads_local(1);if(!sine53_engine_init())return 3;std::string which=argv[1];int rc=2;if(which=="prod"||which=="temp"||which=="intel")rc=run_native(which);sine53_engine_cleanup();return rc;}
