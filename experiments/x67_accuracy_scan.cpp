...
static E load(const char*p){E e{};e.h=dlopen(p,RTLD_NOW|RTLD_LOCAL);if(!e.h){fprintf(stderr,"%s\n",dlerror());std::exit(2);}e.init=(int(*)())dlsym(e.h,"sine53_engine_init");e.eval=(void(*)(double*,const double*,size_t))dlsym(e.h,"sine53_engine_eval");if(!e.init||!e.eval||!e.init())std::exit(3);return e;}
