...
def inject_c0_kgroup32(t):
    old='''        /* Same degree-5 polynomial, algebraically grouped into independent\n           even/odd d^2 chains: much shorter dependency chain than Horner. */\n        for(int g=0;g<4;g++){\n            __m512d z=_mm512_mul_pd(d[g],d[g]);\n            __m512d ec=_mm512_fmadd_pd(z,C24,MH);      /* -1/2 + z/24 */\n            __m512d oc=_mm512_fmadd_pd(z,C120,M6);     /* -1/6 + z/120 */\n            __m512d cd=_mm512_mul_pd(c1[g],d[g]);\n            /* Same polynomial, but fuse the potentially cancelling leading\n               terms first: base = c0 + c1*d.  Corrections are O(z). */\n            __m512d base=_mm512_fmadd_pd(c1[g],d[g],c0[g]);\n            __m512d ep=_mm512_mul_pd(c0[g],ec);\n            __m512d inner=_mm512_fmadd_pd(cd,oc,ep);\n            pv[g]=_mm512_fmadd_pd(z,inner,base);\n            pv[g]=_mm512_mask_sub_pd(pv[g],sg[g],Z,pv[g]);\n            _mm512_storeu_pd(out+i+8*g,pv[g]);\n        }'''
    if t.count(old)!=1: raise SystemExit('kgroup polynomial block count')
    def lane(g,e):
        return f'''        __m512d z{g}=_mm512_mul_pd(d[{g}],d[{g}]);\n        __m512d ec{g}=_mm512_fmadd_pd(z{g},C24,MH);\n        __m512d oc{g}=_mm512_fmadd_pd(z{g},C120,M6);\n        __m512d cd{g}=_mm512_mul_pd(c1[{g}],d[{g}]);\n        __m512d base{g}=_mm512_fmadd_pd(c1[{g}],d[{g}],c0[{g}]);\n        __m512d ep{g}=_mm512_mul_pd(c0[{g}],ec{g});\n        __m512d inner{g}=_mm512_fmadd_pd(cd{g},oc{g},ep{g});\n        pv[{g}]=_mm512_fmadd_pd(z{g},inner{g},base{g});\n        __mmask8 {e}=_mm512_cmp_pd_mask(c0[{g}],_mm512_set1_pd(0x1.921d1fcdec784p-7),_CMP_EQ_OQ);\n        pv[{g}]=_mm512_mask_sub_pd(pv[{g}],sg[{g}],Z,pv[{g}]);\n        _mm512_storeu_pd(out+i+{8*g},pv[{g}]);'''
    hot='''        /* Keep the four edge masks in AVX-512 k registers.  No scalar\n           packing/shifting in the hot path; one kortest gates all 32 lanes. */\n'''+lane(0,'e0')+'\n'+lane(1,'e1')+'\n'+lane(2,'e2')+'\n'+lane(3,'e3')+'''\n        __mmask8 e01=_kor_mask8(e0,e1);\n        __mmask8 e23=_kor_mask8(e2,e3);\n        __mmask8 eany=_kor_mask8(e01,e23);\n        if(__builtin_expect(!_kortestz_mask8_u8(eany,eany),0)){\n            const __m512d DH=_mm512_set1_pd(0x1.921fb54442d18p-7);\n            const __m512d DL=_mm512_set1_pd(0x1.1a62633145c07p-61);\n            const __m512d N5040=_mm512_set1_pd(-1.0/5040.0);\n            const __m512i ABSM67=_mm512_set1_epi64((long long)UINT64_C(0x7fffffffffffffff));\n#define X67_FIX_KGROUP(G,E) do { \\\n            __mmask8 fx=(E); \\\n            if(fx){ \\\n                __m512d ap=_mm512_castsi512_pd(_mm512_and_epi64(_mm512_castpd_si512(pv[G]),ABSM67)); \\\n                fx=(__mmask8)(fx&_mm512_cmp_pd_mask(ap,_mm512_set1_pd(0x1.d14e3bcd35a86p-7),_CMP_GE_OQ)); \\\n                if(fx){ \\\n                    __mmask8 m510=(__mmask8)(_mm512_movepi64_mask(_mm512_castpd_si512(c1[G]))&fx); \\\n                    __m512d b=_mm512_mask_sub_pd(d[G],m510,Z,d[G]); \\\n                    __m512d rh=_mm512_add_pd(DH,b); \\\n                    __m512d re=_mm512_sub_pd(b,_mm512_sub_pd(rh,DH)); \\\n                    __m512d rl=_mm512_add_pd(re,DL); \\\n                    __m512d zz=_mm512_mul_pd(rh,rh); \\\n                    __m512d pp=_mm512_fmadd_pd(zz,N5040,C120); \\\n                    pp=_mm512_fmadd_pd(zz,pp,M6); \\\n                    __m512d alt=_mm512_fmadd_pd(_mm512_mul_pd(rh,zz),pp,rh); \\\n                    alt=_mm512_add_pd(alt,rl); \\\n                    alt=_mm512_mask_sub_pd(alt,sg[G],Z,alt); \\\n                    _mm512_mask_storeu_pd(out+i+8*(G),fx,alt); \\\n                } \\\n            } \\\n        } while(0)\n            X67_FIX_KGROUP(0,e0);\n            X67_FIX_KGROUP(1,e1);\n            X67_FIX_KGROUP(2,e2);\n            X67_FIX_KGROUP(3,e3);\n#undef X67_FIX_KGROUP\n        }'''
    return t.replace(old,hot,1)
...
extra={}
if mode=='baseline':
    # Existing speed workflow uses these legacy filenames: per-8 is the known
    # accurate reference; dd is the new k-register grouped detector candidate.
    extra['direct_tail142']=inject_c0(s,True)
    extra['direct_tail142_dd']=inject_c0_kgroup32(s)
