...
static apple_sine53_earth::PrepareTables tables(){return {apple_sine53_c0,apple_sine53_c1,APPLE_SINE53_LUTN,apple_sine53_pih,apple_sine53_pil,APPLE_SINE53_REDN};}
...
static int validate(){
    B b(9600);fill(b,UINT64_C(202609070053));size_t bad=prep(b);earth(b);apple(b);if(bad){std::printf("SINE53_EARTH_VALIDATE prep_bad=%zu\n",bad);return 4;}
    mpfr_t z,r;mpfr_init2(z,256);mpfr_init2(r,256);
    uint64_t earth_mpfr_max=0,apple_mpfr_max=0,earth_apple_max=0;size_t e_le2=0,e_le3=0,a_le3=0;
    for(size_t i=0;i<b.n;++i){
        ...
        earth_mpfr_max=std::max(earth_mpfr_max,ue);apple_mpfr_max=std::max(apple_mpfr_max,ua);earth_apple_max=std::max(earth_apple_max,uv);
        e_le2+=(ue<=2);e_le3+=(ue<=3);a_le3+=(ua<=3);
    }
    mpfr_clear(r);mpfr_clear(z);
    std::printf("SINE53_EARTH_VALIDATE cases=%zu earth_mpfr_maxulp=%llu apple_vvsin_mpfr_maxulp=%llu earth_vs_vvsin_maxulp=%llu earth_le2=%zu earth_le3=%zu apple_le3=%zu reference=MPFR256\n",b.n,(unsigned long long)earth_mpfr_max,(unsigned long long)apple_mpfr_max,(unsigned long long)earth_apple_max,e_le2,e_le3,a_le3);
    return earth_mpfr_max<=3?0:5;
}

static double median7(double x[7]){std::sort(x,x+7);return x[3];}
static int bench(size_t n){B b(n);fill(b,UINT64_C(202609070054)+n);if(prep(b))return 6;for(int w=0;w<20;++w)earth(b);size_t reps=12000000/(n?n:1);if(reps<3)reps=3;if(reps>20000)reps=20000;double e[7],full[7],av[7];for(int t=0;t<7;++t){uint64_t a=ticks();for(size_t r=0;r<reps;++r)earth(b);uint64_t c=ticks();e[t]=ticks_ns(c-a)/(double)(reps*n);a=ticks();for(size_t r=0;r<reps;++r){prep(b);earth(b);}c=ticks();full[t]=ticks_ns(c-a)/(double)(reps*n);a=ticks();for(size_t r=0;r<reps;++r)apple(b);c=ticks();av[t]=ticks_ns(c-a)/(double)(reps*n);g_sink+=b.y[(t*97)%n];}double em=median7(e),fm=median7(full),am=median7(av);std::printf("SINE53_EARTH_BENCH n=%zu earth_ns_el=%.9f prep_plus_earth_ns_el=%.9f prep_est_ns_el=%.9f vvsin_ns_el=%.9f earth_speedup_vs_vvsin=%.6f full_speedup_vs_vvsin=%.6f reps=%zu sink=%.17g\n",n,em,fm,fm-em,am,am/em,am/fm,reps,(double)g_sink);return 0;}
