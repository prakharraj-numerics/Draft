...
/* Workflow-generated copy of the frozen SINE53 coefficient source.
 * It must be patched to SF_K=8 and SF_LUT_N=403 exactly as the current
 * production workflows do before this bridge is compiled.
 */
...
int sine53_apple_build_c0_c1(double *c0, double *c1, size_t cap)
{
    ...
    for (size_t a = 0; a < (size_t)SF_LUT_N; ++a) {
        ...
        c0[a] = fixed103_to_double(c->coef + 2 * (off + 0),
                                   c->coef_sign[off + 0] != 0);
        c1[a] = fixed103_to_double(c->coef + 2 * (off + 1),
                                   c->coef_sign[off + 1] != 0);
    }
